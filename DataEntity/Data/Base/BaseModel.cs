...
namespace DataEntity.Data.Base
{
    ...
    public abstract class BaseModel : IDataErrorInfo
    {
        ...
        protected virtual string OnValidate(string propertyName)
        {
            ...
            if (!result)                                                   // Pokud validace selhala…
            {
                var validationResult = results.First();                    // vezmeme první chybu
                // FIX: nikdy nevracej null (kompilátor hlásil Possible null reference return)
                error = results.FirstOrDefault()?.ErrorMessage ?? "Neznámá chyba validace.";
